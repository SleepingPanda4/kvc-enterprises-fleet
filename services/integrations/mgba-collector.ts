...
type Options = { fetchImplementation?: typeof fetch; timeoutMs?: number; token?: string };
...
export async function requestMgbaCollection(operationalDate: string, options: Options = {}): Promise<MgbaCollectionResult> {
  const token = options.token ?? process.env.MGBA_WORKER_TOKEN;
  if (!token) throw new MgbaCollectorError("The Monitor worker is not configured.", "MGBA_WORKER_UNCONFIGURED", 503);
  const controller = new AbortController(); const timeout = setTimeout(() => controller.abort(), options.timeoutMs ?? 90_000);
  try {
    let response: Response;
    try { response = await (options.fetchImplementation || fetch)(`${process.env.MGBA_WORKER_URL || DEFAULT_MGBA_WORKER_URL}/collect?date=${encodeURIComponent(workerDate(operationalDate))}`, { method: "POST", headers: { Authorization: `Bearer ${token}` }, signal: controller.signal }); }
    ...
    if (response.status === 423 || body?.skipped === true) throw new MgbaCollectorError("Monitor collection is temporarily blocked; the latest saved snapshot is still available.", typeof body?.code === "string" ? body.code : "MGBA_COLLECTION_BLOCKED", 423);
