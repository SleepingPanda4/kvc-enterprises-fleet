...
export async function getCurrentUser(request: Request) {
  ...
  const db = getDb();
  const [row] = await db.select({ id: users.id, teamMemberId: users.teamMemberId, name: users.name, email: users.email, phoneNumber: users.phoneNumber, role: users.role, verifiedAt: users.verifiedAt })
